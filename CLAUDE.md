...
- **Tailwind CSS v4** — configured CSS-first via `@theme` in `app/globals.css`, not via `tailwind.config.*`. The `@theme` block defines named color tokens (`--color-ink`, `--color-panel`, `--color-copper`, `--color-cream`, `--color-taupe`, `--color-led`, …) and font tokens — but **components mostly use raw inline hex** (`style={{ color: "#E29A5C" }}`) rather than the token-backed utilities. Match whatever the surrounding component already does. Note: components hardcode inline hex, so a full re-theme means remapping the inline hex across every component (a bulk find/replace), not just editing the `@theme` token values.
...
All custom utility classes are defined in `app/globals.css` — **not** as Tailwind plugins. Key ones: `.btn-primary` / `.btn-outline`, `.nav-link`, `.mono-label` / `.section-label` (mono uppercase copper labels), `.font-serif-italic` (Fraunces italic accent), `.chip` (tech-stack pill), `.glow-dot` (pulsing status dot), `.panel` + `.glyph-stage` + `.scan` + `.readout` (Hero status panel), `.service-card` / `.why-card` / `.ctline` / `.project-card`, plus the a11y helpers `:focus-visible` and `.skip-link`, and the animations `.animate-fadeInUp`, `.animate-fadeIn`, `.animate-pulse-ring`, `.animate-scan`, `.animate-node-pulse`, `.animate-trace-flow` with `.delay-100`…`.delay-600`. If you reference a class, confirm it exists in `globals.css` first — `Packages.tsx` (dead code) references `.pricing-card` / `.pricing-badge` which are **not** defined.
